...
describe("generateMetaDescription", () => {
  ...
  for (const { cityId, areaId, categoryId, languageId } of testCases) {
    it(`generates correct description for ${cityId}, ${areaId}, ${categoryId} in ${languageId}`, () => {
      const result = generateAreaCategoryMetaDescription(cityId, areaId, categoryId, languageId);
  ...
  it.skip("handles edge cases with empty strings", () => {
    expect(generateAreaCategoryMetaDescription("", "", "cafe", "en")).toThrowError();
  });

  it("generates unique descriptions for different categories in the same area", () => {
    const cafe = generateAreaCategoryMetaDescription("tokyo", "ningyocho", "cafe", "en");
    const restaurant = generateAreaCategoryMetaDescription("tokyo", "ningyocho", "sweet", "en");
  ...
  it("generates descriptions with correct length", () => {
    const description = generateAreaCategoryMetaDescription("tokyo", "ningyocho", "shopping", "en");
