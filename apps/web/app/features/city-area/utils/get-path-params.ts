...
type OptionalParams<R extends RequireOptions> = (R extends { area: true }
  ? { area: Area }
  : { area?: Area }) &
  (R extends { category: true } ? { category: Category } : { category?: Category }) &
  (R extends { rank: true } ? { rankingType: RankingType } : { rankingType?: RankingType });
...
export const getPathParams = <R extends RequireOptions>(
  ...
  const { lang: langId, area: areaId, category: categoryId, rank: rankingTypeId } = params;

  // リクエストURLのサブドメインから、都市を判定する
  const url = new URL(request.url);
  const subdomain = url.hostname.split(".")[0];
  const city = cities.find((candidate) => candidate.cityId === subdomain) ?? cities[0];
  ...
  if (lang === undefined) throw new Response(null, { status: 404, statusText: "Not Found" });
  ...
  if (options?.require.category && !category)
    throw new Response(null, { status: 404, statusText: "Not Found" });
  const resolvedCategory = options?.require.category ? (category as Category) : category;
  ...
  if (options?.require.rank && !rankingType)
    throw new Response(null, { status: 404, statusText: "Not Found" });
  const resolvedRankingType = options?.require.rank ? (rankingType as RankingType) : rankingType;
  ...
  if (resolvedRankingType !== undefined) {
    result.rankingType = resolvedRankingType as PathParamsResult<R>["rankingType"];
  }
