...
export const AreaTitle = ({ city, languageId, className, ...props }: AreaTitleProps) => {
  const language = languages.find((lang) => lang.id === languageId) ?? languages[0];

  return (
    <h1 className={cn("flex flex-wrap items-end gap-x-2 text-xl font-bold", className)} {...props}>
