...
- **A 藍（推し）**: `oklch(0.55 0.17 265)` / dark では `oklch(0.7 0.15 265)`。乗換案内・路線図の青。地図サービスとしての信頼感。destructive と最も遠い。
- **B 朱**: `oklch(0.58 0.19 38)` / dark では `oklch(0.68 0.17 38)`。食欲と和の商店街。ただし destructive（27°）と近いため、同時表示の整理が必要。
- **C 苔**: `oklch(0.55 0.13 165)` / dark では `oklch(0.7 0.12 165)`。ローカル・散策の落ち着き。夜の dark と相性が良い。

選定基準は「評価スターと destructive エラー表示が並んでも混同しないこと」。A が無難。

決まったら `--color-brand` として `@theme inline` に追加し、`accent`（現行はただのグレー）は shadcn の意味のまま残す。`primary`（ボタン等の主操作色）は neutral のまま変えない。
