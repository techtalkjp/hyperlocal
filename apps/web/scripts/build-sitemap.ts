...
const shardsBaseUrl = process.env.R2_PUBLIC_URL ?? "https://objects.hyper-local.app";
...
const main = async () => {
  ...
  for (const lang of languages) {
    // area category ranks
    const sitemapAreaCategoryRankContent = generateRankSitemap(origin, lang.id, manifest);
    fs.writeFileSync(
    ...
    const sitemapArticlesContent = generateArticlesSitemap(origin, lang.id, manifest);
    fs.writeFileSync(
    ...
    fs.writeFileSync(path.join(outputDir, `sitemap/place-${lang.id}.xml`), sitemapPlaceContent);
  }

  console.log(`Sitemaps generated successfully from manifest ${manifest.version}.`);
};
