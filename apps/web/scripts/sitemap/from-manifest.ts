...
export const generateRankSitemap = (
  ...
  const urls: string[] = [
    entry(`${origin}${langId === 'en' ? '/' : `/${langId}`}`, lastmod, 1.0),
  ]
  ...
  for (const key of Object.keys(manifest.files)) {
    ...
    urls.push(
      entry(
        `${origin}${langPrefix(langId)}/area/${area}/${category}/${rank}`,
        lastmod,
        0.6,
      ),
    )
  ...
  for (const area of [...areas].sort()) {
    urls.push(
      entry(`${origin}${langPrefix(langId)}/area/${area}`, lastmod, 0.8),
    )
  }
...
export const generateArticlesSitemap = (
  ...
  for (const key of Object.keys(manifest.files)) {
    ...
    urls.push(
      entry(
        `${origin}${langPrefix(langId)}/area/${area}/guide/${scene}`,
        lastmod,
        0.7,
      ),
    )
