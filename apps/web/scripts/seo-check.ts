...
const BROWSER_UA = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) hyperlocal-seo-check/1.0";
const GOOGLEBOT_UA =
...
function extractTag(html: string, re: RegExp, group = 1, fallback = "(なし)"): string {
...
function checkHtml(
  ...
  _status: number,
  html: string,
): Omit<UrlCheck, "url" | "status" | "ms" | "cacheControl" | "cdnCacheControl" | "note"> {
  ...
  const hreflangMatches = [...html.matchAll(/<link[^>]+hreflang=["']([^"']+)["'][^>]*>/gi)].map(
    (m) => m[1],
  );
  return {
    title,
    description,
    canonical,
    hreflangs: hreflangMatches.length > 0 ? hreflangMatches.join(",") : "(なし)",
    robotsMeta,
  };
}
...
async function checkUrl(url: string): Promise<UrlCheck> {
  try {
    ...
    const cdnCacheControl = res.headers.get("cloudflare-cdn-cache-control") ?? "(なし)";
...
const main = async () => {
  ...
  for (const result of childResults) {
    ...
    if (urls.length > 50000) warn.push(`50,000件超の子sitemap: ${child}(${urls.length}件)`);
  ...
  lines.push("| URL | 状態 | 応答 | canonical | hreflang | title | description | robots | cache |");
  lines.push("|---|---|---|---|---|---|---|---|---|");
  for (const r of results) {
    const short = (s: string | number) => String(s).replace(/\|/g, "/").slice(0, 60);
    lines.push(
