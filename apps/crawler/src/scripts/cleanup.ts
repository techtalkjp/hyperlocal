...
export const cleanup = async () => {
  ...
  const ranked = await duckdb.selectFrom("ranked_restaurants").select("url").distinct().execute();
  ...
  for (const place of allPlaces) {
    ...
    n++;
    await db.deleteFrom("placeListings").where("placeId", "==", place.id).execute();
    await db.deleteFrom("localizedPlaces").where("placeId", "==", place.id).execute();
