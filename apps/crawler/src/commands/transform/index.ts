...
export const transform = async () => {
  ...
  await db.schema.createTable("genres").as(db.selectFrom("tabelog_genres").selectAll()).execute();
  ...
  await db.schema.dropTable("tr_tabelog_restaurants_genres").ifExists().execute();
  await db.schema
    ...
    .as(
      db
        ...
        .selectAll()
        .select([sql<string>`unnest(string_split(features->>'ジャンル', '、'))`.as("genre")]),
    )
    .execute();

  // Step 2: レストランデータをジャンル/カテゴリ付きで生成
  consola.info("step 2: レストランデータをジャンル/カテゴリ付きで生成");
  await db.schema.dropTable("tr_restaurants_with_genre_categories").ifExists().execute();
  await db.schema
