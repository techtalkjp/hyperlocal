...
if (values.list) {
  ...
  scenes.forEach((scene) => {
    console.log(`  ${scene.id.padEnd(25)} - ${scene.i18n.ja} (${scene.i18n.en})`);
  });
...
const status = (values.status || articleData.status || "published") as "draft" | "published";
...
if (!area || !scene || !title) {
  ...
  console.log("  --status <status>      Article status: draft|published (default: published)");
  ...
  console.log("  pnpm generate:article --area kiyosumi-shirakawa --scene afternoon-break \\");
...
console.log(`  Web: http://localhost:5174/${language}/area/${areaObj.areaId}/guide/${sceneObj.id}`);
