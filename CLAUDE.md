...
- **packages/**: Shared libraries
  ...
  - **db/**: Database schema (Atlas + Kysely + kysely-codegen)
...
1. **Type Safety**: All packages use TypeScript. Database types are generated by kysely-codegen from the actual database.
...
- `packages/db/schema.sql`: Database schema definition (declarative, managed by Atlas)
- `packages/db/atlas.hcl`: Atlas configuration for local and production environments
